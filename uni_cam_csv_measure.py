...
while True:
    ...
    #if inital value of len(ids) is 0 , make 3 by 0 matrix
    """
    #local은 로보타리움 코드에 들어가는 로봇 위치 행렬... 초기화하는 코드
    if ids is not None:
        local = np.zeros((3,len(ids)))
    else:
        local = np.zeros((3,0))
	"""   
    ...
    if ids is not None:
        for i in range(0,ids.size):
            ...
            # list로 깔끔히 저장하는 것으로 바꾸는게 좋을듯 하다
            # 예를들어, actual_x = [[],[],[],[],[],[] ... ]
            if idr == 0:
                tem_x[0].append(x)
                tem_y[0].append(y)
...
# total_xy -> total_theta 바꾸면 theta 볼 수 있음
